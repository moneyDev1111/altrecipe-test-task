...
const getOneInchAmount = (fromToken: Token, toToken: Token, oneInchRate: bigint, multiplierAmount: string | bigint) => {
	...
	if (fromToken.symbol === 'ETH' || toToken.symbol === 'ETH') {
		...
		const price = formatEther(
			(oneInchRate * BigInt(fromToken.symbol === 'ETH' ? numerator : denominator)) / BigInt(toToken.symbol === 'ETH' ? numerator : denominator)
		);
...
export async function checkPriceImpact(
	...
	const oneInchRate: bigint =
		fromToken.symbol === 'ETH'
			...
			: await oneInchAggrContract.getRate(fromTokenMainnet, toTokenMainnet, toToken.symbol === 'ETH' ? true : false);
	// STOP IF RATE IS 0 ( the params are incorrect, mainly the useWrappers bool )
	if (oneInchRate === 0n) throw new Error('Error getting the 1inch Rate');

	const formatted1InchAmount = parseFloat(getOneInchAmount(fromToken, toToken, oneInchRate, quoteAmount));
